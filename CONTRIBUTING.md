...
```bash
git clone https://github.com/PauloJose-PJ/dio-lab-open-source.git
```
Pressione enter, e uma cópia do seu fork no GitHub será criada localmente.

### 3) Crie uma nova **branch** 
Utilize o comando `git checkout -b` para criar e alternar para a nova branch e nomeie-a como `feat/community/PauloJose-PJ`
> Exemplo: `git checkout -b feat/community/PauloJose-PJ`
...
> Exemplo: `community/PauloJose-PJ.md`
...
Utilize o comando `git add community/PauloJose-PJ.md` para adicionar sua alteração (nesse caso o arquivo markdown criado)  à "staging area" no Git.

### 6) Crie um Commit
Crie um commit e adicione a mensagem indicando a adição do seu perfil:
```bash
git commit -m"feat: add PauloJose-PJ profile"
```
>[!IMPORTANT]
...
Envie as alterações realizadas no seu repositório local para a branch `feat/community/PauloJose-PJ` no seu repositório remoto com o comando:
```bash
git push origin feat/community/PauloJose-PJ
```
>[!WARNING]
