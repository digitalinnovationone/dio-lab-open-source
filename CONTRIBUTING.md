...
```bash
git clone https://github.com/sarasouza-t/dio-lab-open-source.git
```
...
> Exemplo: `git checkout -b feat/community/sarasouzat`
...
> Exemplo: `community/sarasouza-t.md`
...
Utilize o comando `git add community/sarasouza-t.md` para adicionar sua alteração (nesse caso o arquivo markdown criado)  à "staging area" no Git.

### 6) Crie um Commit
Crie um commit e adicione a mensagem indicando a adição do seu perfil:
```bash
git commit -m"feat: add sarasouza-t profile"
```
>[!IMPORTANT]
...
Envie as alterações realizadas no seu repositório local para a branch `feat/community/sarasouza-t` no seu repositório remoto com o comando:
```bash
git push origin feat/community/sarasouza-t
```
>[!WARNING]
