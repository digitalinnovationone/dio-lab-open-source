...
| `fix`          | Corrige um bug ou problema no projeto.                                    | `fix: resolve issue #IssueNumber`      |
| `docs`         | Altera ou atualiza a documentação do projeto.                             | `docs: update README.md`               |
| `style`        | Realiza mudanças na aparência ou formatação, sem alterar a funcionalidade.| `style: add EFFECTNAME to COMPONENT`   |
| `refactor`     | Realiza mudanças no código que não alteram a funcionalidade.              | `refactor: optimize CLASSNAME methods` |
